...
import org.apache.iceberg.rest.RestCatalogAdapterV2.HTTPMethod;
import org.apache.iceberg.rest.RestCatalogAdapterV2.Route;
...
/**
 * The RESTCatalogServlet provides a servlet implementation used in combination
 * with a
 * RESTCatalogAdaptor to proxy the REST Spec to any Catalog implementation.
 */
public class RestCatalogServletV2 extends HttpServlet {
    private static final Logger LOG = LoggerFactory.getLogger(RESTCatalogServlet.class);

    private final RestCatalogAdapterV2 restCatalogAdapter;
    private final Map<String, String> responseHeaders = ImmutableMap.of(HttpHeaders.CONTENT_TYPE,
            ContentType.APPLICATION_JSON.getMimeType());

    public RestCatalogServletV2(RestCatalogAdapterV2 restCatalogAdapter) {
    ...
    protected void execute(ServletRequestContext context, HttpServletResponse response)
        ...
        try {
            ...
            this.restCatalogAdapter.healthContainer.add("output_codes", Integer.toString(response.getStatus()));

        } catch (RESTException e) {
            LOG.error("Error processing REST request", e);
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            this.restCatalogAdapter.healthContainer.add("output_codes",
    ...
    protected Consumer<ErrorResponse> handle(HttpServletResponse response) {
        return (errorResponse) -> {
            ...
            this.restCatalogAdapter.healthContainer.add("output_codes", Integer.toString(errorResponse.code()));
            try {
