...
import org.apache.iceberg.core.AuthException;
...
public class RestCatalogAdapterV2 implements RESTClient {
    private static final Logger LOG = LoggerFactory.getLogger(RestCatalogAdapterV2.class);
    ...
    protected final HealthContainer healthContainer = new HealthContainer();

    public RestCatalogAdapterV2(Catalog catalog) {
    ...
    enum Route {
        HEALTH_CHECK(HTTPMethod.GET, "v1/health", null, HealthResponse.class),
    ...
    public <T extends RESTResponse> T handleRequest(
            Route route, Map<String, String> vars, Object body, Class<T> responseType, String userId) {
        healthContainer.add("input_by_route", route.name());
        if (userId != null) {
            healthContainer.add("input_by_users", userId);
        }
        switch (route) {
            case TOKENS: {
                ...
                if (isAuthSystemEnabled()) {
                    checkBasicAuth(request.get("client_id"), request.get("client_secret"));
                    String token = JWebToken.buildToken(request.get("client_id"),
                            getPasswordHash(request.get("client_id")));
                    switch (grantType) {
            ...
            case HEALTH_CHECK:
                Boolean dbCheck = null;
                if (catalog instanceof CheckDb) {
                    dbCheck = ((CheckDb) catalog).checkConnection();
                }
                return castResponse(responseType, healthContainer.buildResponse(dbCheck));
            ...
            case UPDATE_NAMESPACE:
                if (asNamespaceCatalog != null) {
                    Namespace namespace = namespaceFromPathVars(vars);
                    UpdateNamespacePropertiesRequest request = castRequest(UpdateNamespacePropertiesRequest.class,
                            body);
                    return castResponse(
                            responseType,
            ...
            case CREATE_TABLE: {
                ...
                request.validate();
                if (catalog instanceof Loggable) {
                    ((Loggable) catalog).addLog(
            ...
            case DROP_TABLE: {
                if (catalog instanceof Loggable) {
                    ((Loggable) catalog).addLog(identFromPathVars(vars), userId, "drop_table");
                }
            ...
            case LOAD_TABLE: {
                if (catalog instanceof Loggable) {
                    ((Loggable) catalog).addLog(identFromPathVars(vars), userId, "load_table");
                }
            ...
            case UPDATE_TABLE: {
                TableIdentifier ident = identFromPathVars(vars);
                if (catalog instanceof Loggable) {
                    ((Loggable) catalog).addLog(ident, userId, "update_table");
                }
            ...
            case RENAME_TABLE: {
                ...
                if (catalog instanceof Loggable) {
                    ((Loggable) catalog).addLog(request.source(), userId, "rename_from");
                    ((Loggable) catalog).addLog(request.destination(), userId, "rename_to");
                }
            ...
            case REPORT_METRICS: {
                ...
                LOG.info("report metrics: %s", body);
    ...
    /**
     * This is a very simplistic approach that only validates the requirements for
     * each table and does
     * not do any other conflict detection. Therefore, it does not guarantee true
     * transactional
    ...
    public boolean isAuthSystemEnabled() {
        if (catalog instanceof CheckUser) {
            return ((CheckUser) catalog).isAuthRequired();
        }
        return false;
    }

    public boolean requiredAuth(Route route) {
        if (route == Route.HEALTH_CHECK) {
            return false;
        }
        if (route == Route.TOKENS) {
            return false;
        }
        return isAuthSystemEnabled();
    }

    public String checkBasicAuth(String userId, String password) {
        if (catalog instanceof CheckUser) {
            if (((CheckUser) catalog).checkUser(userId, password)) {
                return userId;
            }
        }
        return null;
    ...
    public String getPasswordHash(String userId) {
        if (catalog instanceof CheckUser) {
            return ((CheckUser) catalog).getPasswordHash(userId);
        }
        return null;
    }

    public String checkAuth(Map<String, String> headers) {
        ...
        if (authorization != null && authorization.toLowerCase().startsWith("basic")) {
            ...
            if (checkBasicAuth(values[0], values[1]) != null) {
                return values[0];
            }
        }
        if (authorization != null && authorization.toLowerCase().startsWith("bearer")) {
            String data = authorization.substring("Bearer".length()).trim();
            try {
                return JWebToken.extractUserFromToken(data, ((CheckUser) catalog)::getPasswordHash);
            } catch (RuntimeException exc) {
                LOG.error("got error", exc);
    ...
    public <T extends RESTResponse> T execute(
        ...
        if (routeAndVars != null) {
            try {
                ...
                if (requiredAuth(routeAndVars.first())) {
                    try {
                        userId = checkAuth(headers);
                        LOG.info("got request from: {}, {}", userId, routeAndVars.first().name());
                    } catch (AuthException exc) {
                        errorBuilder
                                .responseCode(401)
                                .withType("AuthException")
                                .withMessage(String.format("Could not found auth"));
                ...
                return handleRequest(routeAndVars.first(), vars.build(), body, responseType, userId);
    ...
    public void close() throws IOException {
        // The calling test is responsible for closing the underlying catalog backing
        // this REST catalog
        // so that the underlying backend catalog is not closed and reopened during the
        // REST catalog's
